...
```bash
npx tsx .agents/scripts/create-justification.ts "$ARGUMENTS" "Your justification content here"
```
...
```bash
npx tsx .agents/scripts/create-justification.ts "$ARGUMENTS" << HEREDOC
Your multi-line
...
```bash
echo "Your content" | npx tsx .agents/scripts/create-justification.ts "$ARGUMENTS"
```
