...
```bash
npx tsx .agents/scripts/create-scratch.ts "$ARGUMENTS" "Your scratch content here"
```
...
```bash
npx tsx .agents/scripts/create-scratch.ts "$ARGUMENTS" << HEREDOC
Your multi-line
scratch content
goes here
HEREDOC
...
```bash
echo "Your content" | npx tsx .agents/scripts/create-scratch.ts "$ARGUMENTS"
```
