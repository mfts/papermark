...
```bash
npx tsx .agents/scripts/create-plan.ts "$ARGUMENTS" "Your plan content here"
```
...
```bash
npx tsx .agents/scripts/create-plan.ts "$ARGUMENTS" << HEREDOC
Your multi-line
plan content
goes here
HEREDOC
...
```bash
echo "Your content" | npx tsx .agents/scripts/create-plan.ts "$ARGUMENTS"
```
