...
import { Folder, PrismaPromise } from "@prisma/client";


/** Could handle multiple folders to move to new single parent folder. In app we ideally use this to move single folder into another.
 * User can move folder to anywhere except to move a parent folder into childFolder
  */
export default async function handle(
  ...
  res: NextApiResponse,
    ...
    if (req.method === "PATCH") {
        // This is able to tackle both scenarios:
        // 1. every passed folderId point to the same parentFolder (At the moment)
        // 2. passed foldersIds have different parentFolders (At the moment)
        // 3. if this api returns 202, then all passed folder will have a common parentFolder
        
        const {userId, teamId} = ids;
        ...
        let requestedFoldersToBeMoved: Folder[] = []; // in here we will store folders whose id is in folderIds
        ...
        if (destinationParentFolderIsRoot){
            ...
            const foldersWhoExistAtRoot = await prisma.folder.findMany({
                where: {
                    parentId: null,
                    teamId
                }
            ...
            requestedFoldersToBeMoved = await prisma.folder.findMany({
                where: {
                    id: {in: folderIds},
                    teamId
                }
        ...
        const foldersRecord : {
            [path:string]: {
                ...
                callbackFnUpdate: (folderToUpdate:Folder)=>PrismaPromise<Folder>
            }
        } = Object.fromEntries(
