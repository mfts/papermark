...
export default async function handle(
  ...
  if (req.method === "GET") {
    // GET /api/teams/:teamId/pitchdecks
    ...
    try {
      ...
      const pitchdecks = team.pitchdecks;
    ...
    // POST /api/teams/:teamId/pitchdecks
    const session = await getServerSession(req, res, authOptions);
    if (!session) {
    ...
    try {
      ...
      // skip triggering convert-pdf-to-image job for "notion" / "excel" pitchdecks
      if (type === "pdf") {
