...
export function normalizeBlockListEntry(entry: string): string {
...
export function isEmailBlocked(email: string, blockListEntry: string): boolean {
    if (!email || !blockListEntry) {
        return false;
    }

    const normalizedEmail = email.trim().toLowerCase();
    const normalizedEntry = normalizeBlockListEntry(blockListEntry);
