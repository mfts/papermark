...
export function encryptSlackToken(token: string): string {
  ...
  const encryptionKey = process.env.NEXT_PRIVATE_SLACK_TOKEN_KEY || process.env.NEXT_PRIVATE_DOCUMENT_PASSWORD_KEY;
  if (!encryptionKey) {
    throw new Error("NEXT_PRIVATE_SLACK_TOKEN_KEY environment variable is required for token encryption");
  }
...
export function decryptSlackToken(encryptedToken: string): string {
  ...
  const encryptionKey = process.env.NEXT_PRIVATE_SLACK_TOKEN_KEY || process.env.NEXT_PRIVATE_DOCUMENT_PASSWORD_KEY;
  if (!encryptionKey) {
    throw new Error("NEXT_PRIVATE_SLACK_TOKEN_KEY environment variable is required for token decryption");
  }
