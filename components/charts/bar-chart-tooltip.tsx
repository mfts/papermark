...
const CustomTooltip = ({
  ...
  const versionNumberStr =
    payload && payload.length > 0 ? payload[0]?.payload?.versionNumber : null;

  const pageNumber = pageNumberStr ? parseInt(pageNumberStr, 10) : 0;
  const versionNumber = versionNumberStr ? parseInt(versionNumberStr, 10) : 1;
  ...
  const { data, error, loading } = useDocumentThumbnail(
    ...
    documentId || "",
    !isNaN(versionNumber) ? versionNumber : undefined,
  );
