...
interface ExportVisitorsModalProps {
  teamId: string;
  teamName?: string;
  onClose: () => void;
}

export function ExportVisitorsModal({
  teamId,
  teamName,
  onClose,
}: ExportVisitorsModalProps) {
    ...
    const fetchExistingExports = async () => {
      try {
        ...
        const response = await fetch(
          `/api/teams/${teamId}/visitors/export-visitors`,
          { method: "GET" },
        );
            ...
            if (statusResponse.ok) {
              ...
              if (statusData.status === "COMPLETED" && statusData.isReady) {
                ...
                try {
                  ...
                  link.setAttribute(
                    "download",
                    `${teamName || "team"}_visitors_${new Date().toISOString().split("T")[0]}.csv`,
                  );
  ...
  const downloadExport = async (exportId: string, resourceName: string) => {
    try {
      ...
      link.setAttribute(
        "download",
        `${resourceName || teamName || "team"}_visitors_${new Date().toISOString().split("T")[0]}.csv`,
      );
      link.rel = "noopener noreferrer";
      link.style.display = "none";
  ...
  const handleClose = () => {
    ...
    setVisitorCount(null); // Reset visitor count
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="mx-4 w-full max-w-md rounded-lg bg-white p-6 dark:bg-gray-800">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold">Export Visitors</h3>
          <button
            onClick={handleClose}
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-md bg-gray-50 p-3 text-sm dark:bg-gray-900">
              <p className="mb-2 font-medium text-muted-foreground">
                Exporting all visitors data
              </p>
              <p className="mb-3 text-muted-foreground">
                This export includes detailed view history for each visitor. It
                may take several minutes for teams with many visitors.
              </p>
              {session?.user?.email && (
                <button
          ...
          <div className="space-y-4">
            <div className="text-sm text-gray-600">
              Export all visitors for: {teamName || "your team"}
            </div>
                      ...
                      {exportJob.status === "COMPLETED" && exportJob.result && (
                        <button
                          onClick={() =>
                            downloadExport(
                              exportJob.id,
                              exportJob.resourceName || teamName || "visitors",
                            )
                          }
