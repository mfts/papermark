...
export const BudgetCounter = ({ budget, setBudget }: BudgetCounterProps) => {
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setBudget(Number(event.target.value));
  };

  const filledPercentage = (budget / 50000) * 100;
  ...
  const styles = `
    ...
    input[type=range].budget-counter {
    ...
    input[type=range].budget-counter::-webkit-slider-thumb {
      -webkit-appearance: none;
      height: 15px;
      width: 15px;
      border-radius: 50%;
    ...
    input[type=range].budget-counter::-moz-range-thumb {
      appearance: none;
      height: 15px;
      width: 15px;
      border-radius: 50%;
